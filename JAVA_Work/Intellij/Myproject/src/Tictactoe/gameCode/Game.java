...
public class Game {
    ...
    public static void com(int[][] room) { //랜덤한 칸을 컴퓨터의 칸으로 만드는 함수
    ...
    public static boolean ipCheck(int input, int[][] room) { //플레이어의 값을 입력받는 함수
    ...
    public static void comTurn(int[][] room, int[] draw) {//room과 draw를 입력받음
        System.out.println("\n컴퓨터의 턴입니다.");
        com(room); //컴퓨터의 칸 채우기 진행
        view(room); // 결과 출력
        draw[0]++; // 무승부 카운트 올림
    }

    public static boolean playerTurn(int input, int[][] room, int[] draw) { //comTurn과 비슷하지만 숫자를 추가로 입력받음
        if (!ipCheck(input, room)) { //틀린 값을 입력했다면
            System.out.println("다시 입력하세요."); //경고문 출력 후
            return false; //false
        }
        view(room); //옳은 값이라면 결과 출력
        draw[0]++; //무승부 카운트 올림
        return true;
    }
